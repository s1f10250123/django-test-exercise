...
class TaskModelTestCase(TestCase):
    ...
    def test_is_overdue_past(self):
        due = timezone.make_aware(datetime(2026, 6, 24, 23, 59, 59))
        current = timezone.make_aware(datetime(2026, 6, 25, 0, 0, 0))
        task = Task(title='task1', due_at=due)
        task.save()
...
class TodoViewTestCase(TestCase):
    ...
    def test_index_post(self):
        client = Client()
        data = {'title': 'Test Task', 'due_at': '2026-06-24 23:59:59'}
    ...
    def test_index_get_order_post(self):
        task1 = Task(title='task1', due_at=timezone.make_aware(datetime(2026, 6, 25)))
        task1.save()
        task2 = Task(title='task2', due_at=timezone.make_aware(datetime(2026, 7, 25)))
        task2.save()
        client = Client()
